...
async  function loadKhsTerm() {
  var khsTerm = document.getElementById("khsTerm")
  const response = await fetch("http://localhost/google_login/studi.php?q=khsTerm&nim=" + "");
  const movies = await response.text();
  const node = document.createElement("li");
  khsTerm.appendChild();
}
function loadKhsTerm() {}
function loadKhsTerm() {}
